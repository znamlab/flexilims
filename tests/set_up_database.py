...
sess_rep = flm_sess.post(datatype='session', name='test_session', origin_id=MOUSE_ID,
                         attributes=dict(path='test/session', test_uniq='unique'),
                         strict_validation=False)
rec_rep = flm_sess.post(datatype='recording', name='test_recording',
                        origin_id=sess_rep['id'],
                        attributes=dict(rec_attr='attribute of recording',
                                        path='test/session/recording'),
                        strict_validation=False)
ds_rep = flm_sess.post(datatype='dataset', name='test_dataset',
                       origin_id=rec_rep['id'],
                       attributes=dict(ds_attr='attribute of recording',
                                       path='test/session/recording',
